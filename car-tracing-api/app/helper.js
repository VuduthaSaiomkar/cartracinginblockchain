...
async function getClientForOrg (userorg, username) {
	...
	// This will load a connection profile osver the top of the current one one
...
var getRegisteredUser = async function(username, userOrg,password, isJson) {
	try {
		...
		console.log("iam from attributes generated",user);
		copService
		if (user && user.isEnrolled()) {
			...
			console.log(admins[0].username,admins[0].secret)
			let request = {
   ...
   // Successfully called the Certificate Authority to get the TLS material
			...
			return  client.setUserContext({username: admins[0].username, password: admins[0].secret}).then((admin1)=>{
				return fabric_ca_client.register({
					enrollmentID: username,
					affiliation: userOrg.toLowerCase() + '.department1',
					role:'client',
					enrollmentSecret: password,
					attrs: [{name: "email",value:"saiomkar@gmail.com",ecert:true}]
					// 	{ value: "saiomkar@gmail.com" }
					// ]
				}, admin1).then((data)=>{
					console.log("it good os the register user output",data)
					return fabric_ca_client.enroll({
						enrollmentID: username,
						enrollmentSecret: password,
						profile: 'tls',
						attr_reqs: [{name:'email',optional:true}]
					}).then((data1)=>{
						console.log("it is good is user as enrolled",data1)
						console.log(data1.key.toBytes())
						logger.debug('Successfully got the secret for user %s',username);
						//client.setTlsClientCertAndKey(data1.key.toBytes(), data1.certificate);
						return client.createUser(
							{username: username,
							mspid: 'companyMSP',
							cryptoContent: { privateKeyPEM: data1.key.toBytes(), signedCertPEM: data1.certificate }
							});
					   }).then((user) => {
						let	member_user = user;
						return client.setUserContext(member_user).then(()=>{
   ...
   // set the material on the client to be used when building endpoints for the user
			...
			//let adminUserObj = await client.setUserContext({username: admins[0].username, password: admins[0].secret});
			//console.log("i am from this ",adminUserObj)
